...
function mostrarDatosEnPerfil() {
  ...
  const albumes = document.getElementById('cant-albums')
  albumes.innerHTML = ""

  if (albumesFamosos.length === 1){
    albumes.innerHTML = `${albumesFamosos.length} álbum`
  } else {
    albumes.innerHTML = `${albumesFamosos.length} álbumes`
  }
  

  const favoritos = document.getElementById('cant-favoritos')
  favoritos.innerHTML = ""
  
  
  const cantLikes = albumesFamosos.filter(album => album.like)
  if (cantLikes.length === 1){
    favoritos.innerHTML = `${cantLikes.length} favoritos`
  } else {
    favoritos.innerHTML = `${cantLikes.length} favoritos`
  }
  ...
  // })
  // if (accLikes === 1){
  //   favoritos.innerHTML = `${accLikes} favoritos`
  // } else {
  //   favoritos.innerHTML = `${accLikes} favoritos`
  // }
